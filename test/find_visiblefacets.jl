...
@testset "find_visiblefacets" begin
    ...
    println("========  $(filepath)  ========")
    shape = AsteroidThermoPPhysicalModels.load_shape_obj(filepath; scale=1000, find_visible_facets=true)
    println(shape)

    ##= Icosahedron =##
    filepath = joinpath("shape", "icosahedron.obj")
    println("========  $(filepath)  ========")
    shape = AsteroidThermoPPhysicalModels.load_shape_obj(filepath; scale=1, find_visible_facets=true)
    println(shape)
    ...
    xs, ys, zs = AsteroidThermoPPhysicalModels.concave_spherical_segment(0.4, 0.2; Nx=2^5, Ny=2^5, xc=0.5, yc=0.5)
    shape = AsteroidThermoPPhysicalModels.load_shape_grid(xs, ys, zs; scale=1.0, find_visible_facets=true)
